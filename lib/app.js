...
main =
{
    ...
    master: function () {
        if (cluster.isMaster)
        {
            ...
            var cpu, workers = main.getMaxWorkers();
    ...
    /**
     * determine how many workers to start limited by number of CPUs
     * and controlled by environment variable WORKERS
     * @function app.do.getMaxWorkers
     */
    getMaxWorkers: function () {
        var cpuCount = require("os").cpus().length;

        if (process.env.WORKERS) {
            var workers = parseInt(process.env.WORKERS);
    ...
    warn: function (message)
    {
        ...
        var warn = chalk.red;
        console.warn(warn("worker#" + cluster.worker.id + ": " + message));
    },
    ...
    {
        var worker = "worker#" + cluster.worker.id + ":";
        if (typeof message === "string")
        {
            arguments[0] = worker + " " + message;
        }
        else
        {
            Array.prototype.unshift.call(arguments, worker);
        }
