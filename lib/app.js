        ...
        module.exports = app;
            ...
            if (!process.env.NOLOGGING)
            {
                app.use(logger("dev", { // combined, common, dev, short, tiny
                    immediate: false // log at start in case of crash
                }));
            }
    ...
    respondWithLocation: function (request, response)
    {
        ...
        response
            ...
            .send([
                {
                    host: "192.168.1.1",
                    error: "The address 192.168.1.1 is not in the database."
                }
            ]);
    },

    /**
     * used to simulate an error to see how error handling works
     * @function app:do.invokeErrorHandler
     */
    invokeErrorHandler: function ()
    {
        // TODO dev binding only
